...
def pull(repo_root: Path, adapter: SyncAdapter) -> List[Dict]:
    ...
    for path in _documents(repo_root):
        ...
        base = base_file.read_text(encoding="utf-8") if base_file.is_file() else document.public_body
        merged, conflict = three_way_merge(base, document.public_body, remote.get("body_markdown", ""))
        if not conflict:
            lines = path.read_text(encoding="utf-8").splitlines()
        ...
        results.append({"path": str(path.relative_to(repo_root)), "kind": kind, "conflict": conflict})
    return results
