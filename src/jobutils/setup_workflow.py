...
def ensure_vimrc_registration(
    ...
    # Older setup runs used shell-style markers in Vimrc. Convert them before
    # replacing the managed block so rerunning setup repairs the invalid file.
    existing = existing.replace(BEGIN_MARKER, VIM_BEGIN_MARKER).replace(
        END_MARKER, VIM_END_MARKER
    )
...
def _posix_wrapper(python_path: Path, source_path: Path, command: str) -> str:
...
def install_user_wrappers(
    job_utils_root: Path, user_bin: Path, platform_name: str
) -> Dict[str, Path]:
    ...
    for name in ("jobutils", "jobutils-python", "jobutils-vim"):
        path = user_bin / name
        content = _posix_wrapper(python_path, root / "src", name)
        _write_managed_file(path, content, "# Generated by job-utils setup.")
...
def run_setup(
    ...
    _run_setup_step(
        state_path,
        "wrappers",
        lambda: install_user_wrappers(root, user_bin, wrapper_platform),
    )
