...
def ensure_vimrc_registration(
    ...
    vimrc_path.write_text(_replace_managed_block(existing, block), encoding="utf-8")
    return vimrc_path


def _posix_wrapper(python_path: Path, command: str) -> str:
...
def install_user_wrappers(
    ...
    for name in ("jobutils", "jobutils-python", "jobutils-vim"):
        path = user_bin / name
        path.write_text(_posix_wrapper(python_path, name), encoding="utf-8")
...
def ensure_shell_profile(
    ...
    profile_path.write_text(_replace_managed_block(existing, block), encoding="utf-8")
    return profile_path
...
def run_setup(
    ...
    _run_setup_step(
        ...
        "vim_and_profile",
        lambda: (
            ensure_vimrc_registration(vimrc, snippet, str(paths.venv_python), root),
