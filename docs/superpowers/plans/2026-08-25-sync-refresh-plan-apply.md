...
- [x] **Step 1: Write the failing test** for a remote commit becoming visible after `fetch`, while the local branch and worktree remain unchanged.
- [x] **Step 2: Run the focused Git tests and confirm failure.**
- [x] **Step 3: Implement `gitops.fetch` with argument validation, credential redaction, and remote revision reporting.**
- [x] **Step 4: Run the focused Git tests and confirm success.**
- [x] **Step 5: Commit the Git refresh slice.**
...
- [x] **Step 1: Write failing tests** that require Git refresh status, an observation file, and external fetch results while preserving Markdown/base files.
- [x] **Step 2: Run the focused sync tests and confirm failure.**
- [x] **Step 3: Implement observation persistence and invoke the new Git fetch boundary.**
- [x] **Step 4: Update the check contract to state that refresh metadata is local ignored state; check still never commits, pushes, or changes Markdown/Atlassian.**
- [x] **Step 5: Run focused tests and commit the refresh/check slice.**
...
- [x] **Step 1: Write failing tests** for external-only change → `import`, two-sided change → `conflict`, and unchanged state → no action.
- [x] **Step 2: Run the focused tests and confirm failure.**
- [x] **Step 3: Add observation loading and plan action generation without embedding credentials or unnecessary remote bodies in the plan.**
- [x] **Step 4: Add stale-observation validation to the plan/apply contract.**
- [x] **Step 5: Run focused tests and commit the plan slice.**
...
- [x] **Step 1: Write failing tests** for import application, conflict blocking, and exactly one post-apply commit before push.
- [x] **Step 2: Run focused tests and confirm failure.**
- [x] **Step 3: Implement import application using the existing three-way merge and metadata materialization logic.**
- [x] **Step 4: Remove the pre-apply commit and retain final commit/push behavior.**
- [x] **Step 5: Run focused tests and commit the apply slice.**
...
- [x] **Step 1: Write failing Vim/documentation tests** for the reduced command surface and check confirmation.
- [x] **Step 2: Run focused tests and confirm failure.**
- [x] **Step 3: Remove pull registration/aliases and wire check confirmation and messages.**
- [x] **Step 4: Rewrite daily usage and normative workflow docs to show check → plan → apply.**
- [x] **Step 5: Run focused tests and commit the user workflow slice.**
...
- [x] **Step 1: Run the full unittest suite, Python compilation, shell syntax checks, and `git diff --check`.**
- [x] **Step 2: Inspect staged content for credentials, personal paths, project-specific values, and stale `sync pull` user instructions.**
- [x] **Step 3: Run the sanitize-artifacts review on user-facing docs and revise any process residue.**
- [x] **Step 4: Push the feature branch and update PR #13 without merging it.**
